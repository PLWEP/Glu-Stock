...
complete_infra_00b = f"""# 🏗️ SECTION 2: INFRASTRUCTURE (Firebase, Secrets & Web Fetchers)
...
for i, cell in enumerate(data['cells']):
    if cell['cell_type'] == 'code':
        ...
        # Target the mangled infra cell
        if "class KaggleInfra:" in src or "fallback = ['AALI.JK'" in src or "import tflite_runtime" in src:
            lines = [line + '\n' for line in complete_infra_00b.split('\n')]
            lines[-1] = lines[-1].strip() # Fix trailing newline for last element
...
if modified:
    ...
    print("COMPLETELY RESTORED 00B INFRASTRUCTURE WITHOUT SYNTAX ESCAPE BUGS!")
