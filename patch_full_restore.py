...
complete_infra = f"""# 🏗️ SECTION 2: INFRASTRUCTURE (Firebase, Secrets & Web Fetchers)
...
for f in glob.glob("notebooks/*.ipynb"):
    if "00a" in f or "00b" in f or "01_" in f:
        ...
        modified = False
        # Find cell index 2 (Execution Count 8 usually, where SECTION 2 is/was)
        for i, cell in enumerate(data['cells']):
            if cell['cell_type'] == 'code':
                src = "".join(cell['source'])
                # If it's the mangled cell or the correct cell
                if "fallback = ['AALI.JK'" in src or "class KaggleInfra:" in src:
                    cell_source = complete_infra
                    if "00b" in f:
                        cell_source = cell_source.replace("import numpy as np", "import numpy as np\\ntry:\\n    import tflite_runtime.interpreter as tflite\\nexcept ImportError:\\n    import tensorflow.lite as tflite")
                        
                    # Now correctly split by \n and append \n because it's a list format for Jupyter
                    lines = [line + '\\n' for line in cell_source.split('\n')] # using \n on multiline python string! Fixes bug!
                    # remove trailing \n from last line if necessary, optional for jupyter
                    
                    data['cells'][i]['source'] = lines
                    modified = True
                    break
        ...
        if modified:
            ...
            print(f"COMPLETELY RESTORED & INJECTED SECTION 2 IN: {f}")
